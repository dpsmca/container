...
COPY --from=0 /wineprefix64/drive_c/rawExtract /wineprefix64/drive_c/rawExtract
COPY --from=0 ${prog32} ${prog32}/
COPY --from=0 ${prog64} ${prog64}/

ENV CONTAINER_GITHUB=https://github.com/dpsmca/container

LABEL description="Convert MS RAW vendor files to open formats or analyze them with Skyline."
LABEL website=https://github.com/dpsmca/container
LABEL documentation=https://github.com/dpsmca/container
LABEL license=https://github.com/dpsmca/container
LABEL tags="Metabolomics,Proteomics,MassSpectrometry,MprcExtractRaw"

ENV WINEDEBUG -all
ENV WINEPATH "C:\pwiz;C:\pwiz\skyline;c:\rawExtract;c:\rawExtract\0.6;c:\rawExtract\bin"
